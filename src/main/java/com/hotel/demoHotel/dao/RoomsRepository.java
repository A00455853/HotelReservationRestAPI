...
public interface RoomsRepository extends JpaRepository<Rooms,Integer> {
    ...
    @Query(nativeQuery=true, value ="select * from hotelroom r join hoteldetail h on h.id = r.hotel_id where  h.id=:hotelid and r.id not in (select b.room_id from bookingdetails b where STR_TO_DATE(:fromdate,'%Y-%m-%d') BETWEEN b.date_from AND b.date_to and STR_TO_DATE(:todate,'%Y-%m-%d') BETWEEN b.date_from AND b.date_to)")
    List<Rooms> findAvailableHotel(@Param("hotelid") int hotelid, @Param("fromdate") String fromdate, @Param("todate") String todate);
